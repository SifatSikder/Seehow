...
def convert_box(cnts):
    ...
    if cnts is not None:
        for c in cnts:
            # Ensure the contour is not empty and has the correct shape
            if c is None or len(c) == 0:
                continue
            c = np.array(c)
            if c.ndim != 3 or c.shape[1] != 1 or c.shape[2] != 2:
                print(f"Skipping invalid contour shape: {c.shape}")
                continue
...
def start():
  ...
  while(cap.isOpened()):
    ...
    if len(boxes_nms):
      ...
      cv2.imwrite(f"{output_path}/{folder_name}/{parts[0]}/{pos:05}.jpg", img_A)  
      cv2.imwrite(f"{output_path}/{folder_name}/{parts[1]}/{pos:05}.jpg", img_D) 
      cv2.imwrite(f"{output_path}/{folder_name}/{parts[2]}/{pos:05}.jpg", frameA)  
      cv2.imwrite(f"{output_path}/{folder_name}/{parts[3]}/{pos:05}.jpg", frameD) 
